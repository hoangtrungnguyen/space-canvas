...
void main() {
  ...
  group('ConnectorHandleGestureHandler', () {
    late ConnectorHandleGestureHandler handler;
    ...
    setUp(() {
      handler = ConnectorHandleGestureHandler();
    ...
    group('canHandle', () {
      testWidgets('returns false for non-panStart events', (tester) async {
      ...
      testWidgets('returns false when no connector is hit', (tester) async {
        when(
      ...
      testWidgets('returns true when start point is hit', (tester) async {
        when(
          () => mediator.hitTest(any(), filter: any(named: 'filter')),
      ...
      testWidgets('returns true when end point is hit', (tester) async {
        when(
          () => mediator.hitTest(any(), filter: any(named: 'filter')),
        ).thenReturn(testConnector);
      ...
      testWidgets('returns false when middle of connector is hit', (
        tester,
      ) async {
        when(
          () => mediator.hitTest(any(), filter: any(named: 'filter')),
        ...
        await pumpTestWidget(
          tester,
          callback: (context) {
            // Middle point (150, 150), distance to start/end > 10
            final event = createEvent(
    ...
    group('doHandle', () {
      testWidgets('selects start handle when start point hit', (tester) async {
        when(
          () => mediator.hitTest(any(), filter: any(named: 'filter')),
        ).thenReturn(testConnector);
        ...
        expect(
          ...
          isA<ActiveLayerEvent>().having(
            ...
            'handle',
            ConnectorHandle.start,
          ),
        );
      });

      testWidgets('selects end handle when end point hit', (tester) async {
        when(
          () => mediator.hitTest(any(), filter: any(named: 'filter')),
        ).thenReturn(testConnector);
        ...
        verify(
          () =>
              mediator.selectConnectorAt(const Offset(200, 200), isDrag: true),
        ).called(1);
